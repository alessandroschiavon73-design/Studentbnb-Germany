(function () {
  ...
  function listingCard(record) {
    ...
    article.innerHTML = `
      ...
      <div class="listing-main"><div class="listing-title-row"><h3><a href="${cfg.routes.listing}?id=${encodeURIComponent(record.id)}">${record.title}</a></h3>${record.is_demo ? `<span class="demo-badge">${demoText.badge}</span>` : `<span class="verification-badge">✓ ${text("verifiedEmail")}</span>`}</div><div class="listing-meta"><span>⌖ ${city.name} · ${record.district_name}</span><span>▣ ${record.available_from}</span></div><div class="listing-submeta"><span>${record.arrangement}</span><span>1 Woche · 2 Wochen · 1 Monat</span></div></div>
      <div class="listing-price"><div class="price">${formatMoney(stayPrices.p7)} <small>/7 Tage</small></div><div class="test-price-breakdown"><span>14 Tage ${formatMoney(stayPrices.p14)}</span><span>30 Tage ${formatMoney(stayPrices.p30)}</span></div><span class="expenses-badge ${record.expenses_included ? "included" : "excluded"}">${record.expenses_included ? text("expensesIncluded") : text("expensesExcluded")}</span></div>
  ...
  function initCityPage() {
    ...
    const cityDescription = `${selectedCity.name}: Teste Wohnung, Viertel und das Zusammenleben für eine Woche, zwei Wochen oder einen Monat, bevor du dich entscheidest.`;
    q("#city-description").textContent = cityDescription;
    document.title = `${selectedCity.name} — 1 Woche, 2 Wochen oder 1 Monat | StudentBnB`;
  ...
  function initListing() {
    ...
    q("#listing-price").innerHTML = `${formatMoney(stayPrices.p7)} <small>/7 Tage</small><span class="detail-test-prices"><span>14 Tage ${formatMoney(stayPrices.p14)}</span><span>30 Tage ${formatMoney(stayPrices.p30)}</span></span><span class="monthly-reference">Monatliche Referenzmiete: ${formatMoney(stayPrices.monthly)}</span>`;
    ...
    q("#listing-minimum").textContent = "1 Woche · 2 Wochen · 1 Monat";
